...
mixi_font = pg.font.Font(None, 200)
...
class Item:
    def __init__(self, name, price, file, is_using, is_bought):
        ...
        self.is_bought = is_using
        self.is_using = is_bought
...
class ClothesMenu:
    ...
    def buy(self):
        if self.game.money >= self.items[self.current_item].price:
            self.game_money -= self.items[self.current_item].price
...
class Food:
    def __init__(self, name, price, file,  satiety, medicine_power=0):
        ...
        self.madicine_power = medicine_power
...
class Dog(pg.sprite.Sprite):
    def __innit__(self):
        ...
        self.background = load_image("images/dog.png", DOG_WIDTH // 2, DOG_HEIGHT // 2)
        self.rect = self.image.get_rect()

        self.rect.centrerx = SCREEN_WIDTH // 2
        self.rect.centrerx = SCREEN_HEIGHT - MENU_NAV_YPAD - 10
...
class MiniGame:
    ...
    def update(self):
        ...
        self.score += len(hits)
        if pg.time.get_ticks() - self.start_time > self.iterval:
...
class Game:
    def __init__(self):
        ...
        with open("./save.json") as f: data = json.load(f)
        ...
        self.upgrate_button = Button("Улучшить", SCREEN_WIDTH - ICON_SIZE, 0,
                                     width=BUTTON_WIDTH // 3, height=BUTTON_HEIGHT // 3,
                                     text_font=mini_font,
                                     func=self.increase_money)

        self.buttons = [self.eat_button, self.clothes_button, self.play_button, self.upgrate_button]
        ...
        self.INCCREASE_COINS = pg.USEREVENT + 1
        pg.time.set_timer(self.INCCREASE_COINS, 1000)
        self.DECREASE = pg.USEREVENT + 2
        pg.time.set_timer(self.DECREASE, 100)

        self.NEW_DAY = pg.USEREVENT + 2
        pg.time.set_timer(self.NEW_DAY, 24 * 60 * 60 * 1000)
    ...
    def event(self):
        for event in pg.event.get():
            ...
            if event.type ==self.INCCREASE_COINS:
    ...
    def draw(self):
        ...
        self.screen.blit(text_render(self.happiness), (PADDING + ICON_SIZE, PADDING * 6.3))
        self.screen.blit(text_render(self.satiety), (PADDING + ICON_SIZE, PADDING * 6.3 * 3.7))
        self.screen.blit(text_render(self.health), (PADDING + ICON_SIZE, PADDING * 6.3* 6.25))
        ...
        if self.mode == "Game over":
            text =mixi_font.render("ПРОИГРЫШЬ", True, "red" )
